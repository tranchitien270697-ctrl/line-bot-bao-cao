...
app.get('/api/loss', async (req, res) => {
  try {
    ...
    console.error('loss list fetch error', e);
    res.status(500).json({ error: 'loss list fetch failed' });
  }
});
...
app.get('/api/mooncake', async (req, res) => {
  try {
    const url = CUSTOMER_SHEET_URL + '?key=' + CUSTOMER_SHEET_KEY + '&action=mooncake_list';
    const r = await fetch(url);
    const data = await r.json();
    res.json(data);
  } catch (e) {
    console.error('mooncake list fetch error', e);
    res.status(500).json({ error: 'fetch failed' });
  }
});
...
app.get('/api/product-natafood', async (req, res) => {
  try {
    const url = CUSTOMER_SHEET_URL + '?key=' + CUSTOMER_SHEET_KEY + '&action=product_natafood_list';
    const r = await fetch(url);
    const data = await r.json();
    res.json(data);
  } catch (e) {
    console.error('product natafood list fetch error', e);
    res.status(500).json({ error: 'fetch failed' });
  }
});

app.get('/api/mooncake-stock', async (req, res) => {
  try {
    const url = CUSTOMER_SHEET_URL + '?key=' + CUSTOMER_SHEET_KEY + '&action=mooncake_stock_list';
    const r = await fetch(url);
    const data = await r.json();
    res.json(data);
  } catch (e) {
    console.error('mooncake stock list fetch error', e);
    res.status(500).json({ error: 'fetch failed' });
  }
});
...
app.get('/api/revenue/:date/products', async (req, res) => {
  try {
    const url = CUSTOMER_SHEET_URL + '?key=' + CUSTOMER_SHEET_KEY + '&action=revenue_products&date=' + encodeURIComponent(req.params.date);
    const r = await fetch(url);
    const data = await r.json();
    res.json(data);
  } catch (e) {
    console.error('revenue products fetch error', e);
    res.status(500).json({ error: 'revenue products fetch failed' });
  }
});
